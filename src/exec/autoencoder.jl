...
function hess_vec(args,NN,input_,acts, xb,xt)
    ...
    n_ = args["x_dim"]
    p_ = args["par_dim"]
    dxx = reshape(0.0f0 .* xb, n_, tdim*p_)
    for j = 1:n_
        for k = 1:n_
...
function build_loss(args,dzdt_rhs,dzdt_sens_rhs,encoder, decoder, par_encoder,par_decoder)    
    function loss_(in_,dx_,par_,dxda,dtdxda)
        tdim = args["tsize"]
        n_ = args["x_dim"]
        p_ = args["par_dim"]
        # AE loss
        ...
        loss_dzdt = args["P_cons_z"]*Flux.mse(dz1,dzdt_rhs(enc_,enc_par,0.0f0))
        loss_AE_par = args["P_AE_par"]*Flux.mse(dec_par,par_)
        loss_sens_dtdzdb = args["P_sens_dtdzdb"]*Flux.mse(dtdzdb,dtdzdb_data)
        loss_sens_x = args["P_sens_x"]*Flux.mse(dxda,dxda_decode)
        ...
        loss_total = loss_AE_state + loss_dxdt + loss_dzdt + loss_AE_par + loss_sens_dtdzdb + loss_sens_x
